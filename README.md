...
- **어댑터는 업데이트와 무관하다** — `.claude/iterate.config.md`(ROLE_* 포함)는 대상 레포에 있으므로 플러그인을 갱신해도 그대로 유지된다. 단 새 버전이 어댑터 필드를 추가했으면(CHANGELOG/커밋 메시지 참조) `examples/iterate.config.md` 를 보고 해당 필드만 보강하라.
