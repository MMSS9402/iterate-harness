...
examples/iterate.config.md       # 어댑터 템플릿 (Flutter 앱·Go 백엔드 예시 2종)
```
...
C reviewer     테스트가 놓친 결함 사냥 (명세→기대 도출→반례 증거)
   ├ 갭 ≥1 → 결함은 test-author 케이스로 환원 → 게이트 재진입 / 중복·죽은코드는 implementer 정리
   └ 갭 0 → ★ 자동 green 확정
...
- **재동결은 매 G2 escapes-0 직후에만.** 재실행에서 test-author 가 정당하게 케이스를 추가하면 G2 재심사 통과 후 재동결된다. 그 외 구간(implementer 단계)의 테스트 변경만 변조로 판정 — 정당 수정과 변조를 시점으로 구별한다.
...
1. `examples/iterate.config.md` 를 대상 레포의 `.claude/iterate.config.md` 로 복사하고, 각 필드에서 당신 스택에 맞는 예시 블록(Flutter/Go 중)만 남긴 뒤 값을 프로젝트에 맞춘다.
...
- 두 예시(Flutter 모바일 앱·Go 백엔드)가 UI 있는/없는 스택, 원장 있는/없는 운영을 모두 보여준다.
