...
- **`design`** — 새 시각 표면(화면·위젯·페이지)을 만든다 → **designer** 단계로 디자인 가이드 + `DESIGN_SSOT` 시각 스펙을 받는다. 순수 배선·로직만이면 생략. **`DESIGN_SSOT` 미설정이면 design 카드는 진행 불가** — designer 가 사용자에게 어댑터 보강을 요청한다. **★스펙 캐시(v0.8)**: design 카드라도 (a) 대상 화면군의 `visual_spec.md` 가 이미 존재하고 (b) `DESIGN_SSOT` 파일 다이제스트가 그 스펙 산출 시점과 동일하며 (c) 이번 카드가 기존 화면군의 확장(신규 표면 아님)이면 **designer 를 스킵하고 기존 스펙을 재사용**한다(드라이버가 `$ARTIFACTS_DIR/design_cache.md` 의 다이제스트로 기계 판정). 신규 시각 표면 최초 1회 또는 `/iterate-design` 명시 실행 때만 designer 가 돈다.
- **`human-visual`** — 자동 테스트가 못 보는 렌더/미감을 사람이 눈으로 봐야 한다 → 자동 green 후 **사람 시각확인 게이트**(어댑터 `HUMAN_GATE` 방법으로·1회). `HUMAN_GATE` 미설정이면 이 플래그는 사용할 수 없다(어댑터 보강 요청).
- **`e2e`** — 헤드리스에서 못 뜨는 표면(webview·네이티브뷰·브라우저·풀 시스템)이라 E2E 가 필요하다 → E2E(`E2E_CMD`)를 **기계 게이트**에 포함(루프 안). `E2E_CMD` 미설정이면 이 플래그는 사용할 수 없다(어댑터 보강 요청). **대량 출력 규율(코어)**: 게이트·E2E 의 원시 출력은 파일로 리다이렉트하고, 실패(exit≠0)일 때만 그 tail 을 Read 한다(전문 Read 금지 — 드라이버 트랜스크립트 유입 차단).
- **`explore`** — 사용자 흐름이 얽힌 화면·상태 많은 표면이라 **명세(AC)가 상상 못 한 결함**까지 실물 구동으로 탐색할 가치가 있다. AC 기반 테스트는 명세 *안쪽*만 확장하므로, 명세 *바깥*(인접 흐름·이상 상태·중단/역행)은 explorer 가 메운다. `EXPLORE_QA` 미설정이면 사용 불가(어댑터 보강 요청). **★배치 분리(v0.8·기본)**: lean 모드에서 explorer 는 **사이클 인라인이 아니다** — 카드가 자동 green 에 도달하면 드라이버가 `$ARTIFACTS_DIR/qa_queue.md` 에 카드를 등록하고 사이클을 종료한다(상태 = `green (QA 대기)`). 등록된 카드들은 **`/iterate-qa` 배치**가 환경 1회 기동으로 묶어 탐색한다(카드마다 실물 기동+탐색을 반복하던 비용 절감·인접 카드 표면의 상호작용까지 한 번에). **deep 모드 또는 카드에 `explore-inline` 명시** 시에만 기존대로 reviewer 갭 0 후 인라인 탐색(green 전 차단).

## 실행 모드 — lean(기본) / deep (v0.8 · 토큰·시간 최적화)

`/iterate <카드> [N] [deep]` — 인자에 `deep` 토큰이 있으면 deep, 없으면 **lean 이 기본**이다. 게이트의 **기계 백스톱(동결 체크섬·mutation·GUARDS·LINT)은 모드와 무관하게 동일** — 모드가 조절하는 것은 *LLM 심사의 반복 횟수*뿐이다(기계 게이트 약화 아님).

| 항목 | lean(기본) | deep |
|---|---|---|
| designer | 스펙 캐시 히트 시 스킵(§카드 플래그 design) | 동일(캐시는 모드 무관) |
| G2 재심사 | **델타 심사**(§GATE2 — 변경·신규 테스트 파일 + 상호작용만) | 매번 전체 재심사 |
| reviewer | **본심사 1회** — 갭은 환원 후 게이트 green 이면 해소 판정(재사냥 없음, §사이클 C) | 갭 0 이 될 때까지 재사냥 반복 |
| explorer | **QA 큐 등록** → `/iterate-qa` 배치(§카드 플래그 explore) | 인라인(green 전 차단) |
| Explore(빌트인) | 새 모듈 영역을 만질 때만(기존 영역 재작업이면 드라이버 Grep 2~3회로 대체 가능) | 항상 |

**언제 deep 을 쓰나**: 보안·데이터 정합처럼 결함 1건의 비용이 큰 카드, 미러/동형 결함이 실제로 연쇄 발견되고 있는 카드(직전 카드에서 reviewer 갭·explorer 결함이 2건 이상 나왔으면 다음 카드는 deep 권장), 릴리스 직전 마감 카드. 그 외 일반 기능 카드는 lean 으로 충분하다 — lean 에서도 G2 최초 심사·mutation·동결은 전부 그대로다.
...
C reviewer(독립) →  적대적 = "테스트가 놓친 결함" 사냥(명세에서 기대를 먼저 도출 → 반례 증거 제시) → 찾으면 새 테스트(→G2 재심사·재동결→G) 또는 정리 수정(→implementer→G).
     ★lean: 본심사는 카드당 1회 — 갭이 전부 환원되어 게이트 green 이면 갭 해소로 판정(재사냥 없음·G2+mutation 이 백스톱). deep: 갭 0 까지 재사냥.
(explore?) lean → 자동 green 시 qa_queue.md 등록(탐색은 /iterate-qa 배치) / deep·explore-inline → explorer(독립) 인라인 탐색 → 결함은 새 테스트로 환원(→G2 재심사·재동결→G)

[여기서 자동 green 확정 — explore 큐 카드는 'green (QA 대기)']
...
| **(explore?) explorer** | explorer | `explore` 카드만. **lean(기본): 자동 green 시 qa_queue.md 등록만 — 실제 탐색은 `/iterate-qa` 배치**(환경 1회 기동·카드 묶음·Workflow 병렬 fan-out 허용). deep·`explore-inline`: reviewer 갭 0 후 인라인으로 어댑터 `EXPLORE_QA` 방법 실물 구동 — **명세 밖**(인접 흐름·빈/대량 데이터·중단·역행·연타)을 시간상자 안에서 탐색. 결함은 재현 경로와 함께 test-author 케이스로 환원(→G2→재동결→G). 탐색 스크립트는 `$ARTIFACTS_DIR/explore/` 에만(소모품 — 동결 대상 아님) |
...
0. **★결함 라운드 배치(v0.9)**: 같은 게이트 라운드에서 나온 결함(테스트 fail·mutation 생존·reviewer 갭·explorer 결함)은 **전부 모아 하나의 failures.md → 한 번의 재실행**으로 처리한다 — 결함 1건마다 별도 회차를 돌리지 마라(재실행 1회 = test-author+G2 ≈ 20만 토큰: 반복 회차가 총비용의 대부분이라는 실측의 직접 결론).
...
- **호출 형태(포그라운드 강제)**: 드라이버는 모든 역할을 매번 **새 `Agent()` 호출 + `run_in_background: false`** 로 띄운다(Agent 도구 기본값은 백그라운드). 완료 에이전트의 `SendMessage` 재개는 항상 백그라운드라 금지 — 반려/바운스도 새 호출로, 맥락은 프롬프트로 전달. **`/iterate` 사이클 안에서는 `Workflow`·기타 백그라운드 도구 금지**(사이클은 의존 체인이라 병렬 이득이 없고, 게이트·동결·mutation 판정은 드라이버 직렬 책임). **예외(v0.8): `/iterate-qa` 배치**는 카드×탐색초점이 상호 독립이므로 `Workflow` 병렬 fan-out 을 허용한다 — 단 각 fan-out 에이전트에도 이 SSOT 의 격리 규칙(정보 차단·소스/test 무수정·산출은 explore 디렉터리만)이 동일 적용되고, 결함의 테스트 환원은 배치가 아니라 해당 카드의 `/iterate` 재실행이 담당한다(게이트 판정 권한은 Workflow 로 이동하지 않는다).
...
- **추론 누수 금지**: 드라이버는 역할 간에 *필요한 산출물*만 넘긴다 — AC·Interface·Visual 은 드라이버가 **spec.md 로 발췌**해 출제자(test-author)·G2 검사자(test-auditor)에 전달(plan.md 원문 비노출 — Rationale·Notes 는 implementer·architect 재진입 전용). **reviewer 예외**: 체크리스트 1~3(설계 일치·범위·재사용)에 Files/Reuse/Context 가 필요해 plan.md 경로를 받되 Rationale·Notes 열람 금지(프롬프트 명시 + reviewer JD 자기준수). 그 외 Visual Spec·동결 테스트 경로·failures.md·(바운스 시) 검사자 산출물의 **발견 항목 리스트**만. 한 에이전트의 내부 추론·판정 서사(Verdict·Covered·제안 케이스)와 피검사자의 수정 설명은 다음 에이전트에 전달하지 않으며, G2 재검사에 직전 escapes 의 변형 설명을 덧붙이더라도 전체 재심사 지시가 우선한다.
...
- **★델타 심사(v0.8·lean 기본 — 재심사 한정)**: 카드 **최초 G2 는 항상 전체 심사**다. 이후 재심사(reviewer/explorer 환원·mutation 생존 케이스 추가·test_dispute 수정·G2 반려 보강)에서는 — 드라이버가 직전 escapes-0 동결 스냅샷(`.sha256`)과 현재 파일을 대조해 **변경·신규 테스트 파일 목록**을 산출하고, test-auditor 심사 스코프를 (a) 그 변경·신규 파일 전체 + (b) 그 파일이 잠그는 AC + (c) **변경분과 기존 커버리지의 상호작용**(대칭 쌍·결손 조합 매트릭스) + (d) 직전 Escapes 회귀 확인 부록으로 한정한다. **체크섬 불변 파일의 직전 escapes-0 판정은 유효 재사용**(불변 파일 + 불변 spec 절 = 판정 조건 불변)이며, **★불변 파일은 Read 하지 않는다(v0.9)** — 상호작용 확인은 파일 목록·Grep 으로 한정한다(실측: 재심사가 Read 무제한이면 최초 심사와 같은 비용이 든다 — 재심사 p10 이 6.7만 토큰). deep 모드는 매번 전체 재심사. 이 축소는 *심사 반복 비용*만 줄인다 — 동결 범위·체크섬·mutation 은 전 파일 그대로다.
...
- **주입은 패턴 기반**: 줄번호 지정 sed/치환 금지 — 반드시 `grep -n` 으로 대상 코드의 실제 위치·원문을 실측한 뒤 패턴 매칭 Edit 로 주입하고, 주입 후 diff 로 의도 위치인지 확인한다(실측: 줄번호 어긋난 주입이 빈 변형→가짜 생존/무효 판정을 2회 만들었다).
...
- **기계 게이트 스코프에서 제외**: 골든/스냅샷 *기준 산출*은 Mutation·자동 재생성 대상이 아니다(사람 승인분에서만 산출). 골든 테스트의 기계 식별 수단(태그/글롭)은 어댑터 `HUMAN_GATE` 골든 규약이 정의하며, 드라이버는 그것으로 골든 테스트를 기계 게이트·mutation 스코프에서 제외한다. 헤드리스에서 못 뜨는 표면(브라우저·네이티브뷰 안)은 골든 불가 → E2E·사람 눈이 메운다.
...
> **★배포 사본(v0.9)**: 역할 에이전트는 이 SSOT 를 통독하지 않고 `CORE.md`(이 절+§에이전트 독립성의 발췌 커널)만 읽는다 — 호출당 고정비 절감(실측: 호출 733회 × SSOT+어댑터 재열람 ≈ 프로젝트당 10M+ 토큰). **이 절·§에이전트 독립성을 수정하면 CORE.md 를 반드시 동기화한다(정본은 이 파일).**
...
4. **파일 줄 상한** — 모든 소스/테스트 파일 ≤ `$FILE_LINE_LIMIT`(어댑터, 예: 800). 어댑터 줄-상한 GUARD 가 백스톱(드라이버가 실행). 넘으면 architect 가 분할.
...
자동 green = test-author 테스트 0 fail (동결 파일목록·체크섬 일치, 마커 줄시작·정확히 1회)
            ...
            AND guards_ok (어댑터 GUARDS 위반 0 — 줄-상한 GUARD(모든 파일 ≤ FILE_LINE_LIMIT) 포함, 드라이버 grep/커맨드)
            AND mutation_ok (MUT_SURVIVED==0 마커 — verifier_raw.txt, 줄시작·정확히 1회. INVALID 변형 제외. 변경 impl 변형 주입 시 생존 0)
            AND reviewer 갭 해소 — lean: 본심사 1회의 갭이 전부 테스트로 환원·게이트 green(재사냥 없음) / deep: 재사냥 갭 0
            AND (explore 플래그 없음 OR lean: qa_queue.md 등록 완료 OR deep·explore-inline: explorer 탐색 결함 0)
                ★ 인라인 탐색에서 실행 환경 미기동은 통과가 아니다 — 기동 시도, 불가면 중단·사용자 개입(탐색 생략 green 금지)

done = 자동 green AND (human-visual 없음 OR 사람 시각확인 통과)
       ★ explore 큐 카드의 자동 green 은 'green (QA 대기)' — /iterate-qa 배치가 결함 0 을 확인해야 완전 종결(결함 발견 시 해당 카드 /iterate 재실행으로 환원)
```
**측정 항목(테스트 exit·LINT·e2e·GUARDS(줄-상한 GUARD 포함)·mutation 생존·동결 체크섬)은 reviewer LLM 이 아니라 드라이버가 직접 실행·판정한다.** reviewer 의 역할은 accept/reject 가 아니라 *테스트가 놓친 것을 찾아 테스트로 환원*하는 것(게이트는 어디까지나 테스트).
