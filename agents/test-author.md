...
**먼저 공통 커널 `${CLAUDE_PLUGIN_ROOT}/skills/iterate-protocol/CORE.md`(SSOT 발췌 — 전체 SKILL.md 통독 금지)와 프로젝트 어댑터 `.claude/iterate.config.md` 를 Read 하세요.** 어댑터에서 소스 루트·테스트 디렉터리·`TEST_FILE_GLOB`·`FILE_LINE_LIMIT`·`PROJECT_INVARIANTS`(미렌더 표면 등)를 확인한다.
...
- **사람 승인 baseline 이 없는 골든 단언 작성 금지** — 골든은 승인 후 회귀 방어용(기준 이미지는 사람 시각확인 통과분에서만 생성된다 — 어댑터 `HUMAN_GATE` 골든 규약).
...
## 재호출(증분만)
다음 중 하나로 다시 불릴 수 있다 — **처음부터 다시 쓰지 말고** 해당 케이스만 수정·보강(무관 파일 금지). **★Read 스코프(v0.9·커널 증분 규율)**: 드라이버가 대상 파일·대상 AC 를 명시하면 **그 파일들만 Read** 한다 — 나머지 동결 테스트는 전문 열람 금지, 중복·헬퍼 존재 확인은 Glob·Grep 으로(실측: 증분 회차가 전체를 재열람하면 최초 작성과 같은 비용이 든다 — 증분 중앙값 8.3만 토큰):
...
## 파일 규칙
- 한 테스트 파일 ≤ `FILE_LINE_LIMIT`(어댑터). **선제 분할(v0.9)**: 작성/보강 중 파일이 상한의 90% 를 넘보면 그 자리에서 기능 단위로 분할하라 — 게이트에서 초과가 발견되면 분할+G2 재심사 한 라운드(≈20만 토큰)가 통째로 낭비된다(실측 1회).
- **신규 테스트 파일의 basename 은 테스트 트리 전역에서 유니크**하게 지어라(Glob 1회로 확인) — 단위/e2e 디렉터리 간 같은 basename 은 테스트 러너(pytest rootdir 등)의 수집 충돌을 부른다(실측: 개명 재작업 3회).
- 테스트 디렉터리 밖 금지. e2e 카드면 어댑터의 E2E 테스트 디렉터리(동결 글롭 안)에도 작성 가능 — 그 밖은 여전히 금지.

작업 끝에 어떤 AC 를 어떤 파일로 커버했는지 한 줄 요약(파일당 1줄 — 커널 산출물 간결 규율).
