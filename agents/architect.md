...
### Card flags — 이 카드의 플래그 판정(SSOT): `design`(새 시각 표면→designer), `human-visual`(사람 눈 필요), `e2e`(헤드리스 미렌더 표면→E2E), `explore`(사용자 흐름이 얽힌 화면·상태 많은 표면 — 명세 밖 탐색 QA 가치). 해당 없으면 "없음(테스트만)". 근거 1줄. (`design` 인데 어댑터 `DESIGN_SSOT` 미설정이면 Open questions 에 어댑터 보강 필요를 명시. `explore` 인데 어댑터 `EXPLORE_QA` 미설정이면 동일하게 Open questions 에 명시.)
...
### Open questions — 모호하거나 사용자 결정 필요한 것 (어댑터 필드 미설정 포함). 각 항목에 `[blocking]`/`[non-blocking]` 표기 — blocking = 이 답 없이는 AC 확정 불가
...
- **테스트 얕음(mutation 생존/reviewer 갭)** → 계획·AC 는 유지하되 Notes 에 변형의 관찰가능 행동을 기록 — 이 경우에도 test-author·G2 는 스킵되지 않는다(드라이버 규칙).
