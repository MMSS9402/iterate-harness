...
## TEST_SUPPORT_GLOBS
테스트 **지원 파일** 패턴 — 헬퍼·픽스처·Fake·테스트 유틸. 동결 스냅샷은 **TEST_FILE_GLOB ∪ TEST_SUPPORT_GLOBS** 매칭 파일 전체다: TEST_FILE_GLOB 은 "실행할 테스트" 열거용이고, 동결 범위는 test-author 가 쓸 수 있는 모든 것(지원 파일 포함)이어야 한다. **test-author 가 이 패턴 밖에 지원 파일을 만들면 드라이버가 반려한다.** 지원 파일이 없으면 `(없음)` 을 쓴다. **E2E_CMD 가 실행하는 E2E 테스트 파일 경로도 반드시 동결 범위(TEST_FILE_GLOB ∪ TEST_SUPPORT_GLOBS) 안에 두라** — 밖이면 implementer 가 e2e 테스트를 약화해도 동결 체크섬·가드가 못 잡는다(예시 C 의 tests/e2e 포함 지시와 동일 원칙).

**예시 A · Flutter** (별도 테스트 트리 — test/ 전체가 test-author 영역이라 트리 통째 + E2E_CMD 가 실행하는 integration_test/ 도 동결. find 변환: `find test integration_test -type f`)
```
test/**
integration_test/**
```
...
## HUMAN_GATE (선택)
`human-visual` 카드의 시각확인 방법 + (있으면) 골든/스냅샷 규약. 방법·골든 두 줄. 없으면 human-visual 플래그 미사용(순수 로직만). **골든 규약을 쓰면 기계 식별 수단(태그 또는 글롭)을 반드시 함께 정의**하라 — 드라이버가 그것으로 골든 테스트를 기계 게이트·mutation 실행에서 제외한다.

**예시 A · Flutter**
```
- 방법: 부팅된 iOS 시뮬레이터에 `flutter run -d ios` 로 띄우고 스크린샷 캡처 → 사용자에게 제시.
- 골든: matchesGoldenFile 사용, 골든 테스트 파일에 `@Tags(['golden'])` 부착(기계 식별 수단). 기준 이미지(--update-goldens)는 사람 시각확인 통과분에서만 생성(드라이버 임의 갱신 금지). 기계 게이트 스코프 제외 — 기계 게이트 실행은 `flutter test --exclude-tags golden`.
```
...
- 골든: playwright `toHaveScreenshot()` 사용 시 기준 스냅샷(`--update-snapshots`)은 사람 시각확인 통과분에서만 생성(드라이버 임의 갱신 금지). 기계 게이트 스코프 제외 — `toHaveScreenshot` 스펙은 별도 글롭(예: `tests/e2e/**/*.visual.spec.ts`)으로 식별(기계 식별 수단)해 기계 게이트·mutation 실행에서 제외.
```

## EXPLORE_QA (선택)
`explore` 플래그 카드의 **탐색 QA** 방법 — AC 기반 테스트가 못 보는 *명세 밖*(인접 흐름·이상 상태·중단/역행·경계 밖 입력)을 explorer 가 실물 구동으로 탐색한다. 세 줄로 적는다: **기동**(실행 환경 띄우는 법) · **조작**(무엇으로 구동하나 — 일회성 탐색 스크립트는 `$ARTIFACTS_DIR/explore/` 에만) · **초점**(이 프로젝트에서 우선 탐색할 영역). 없으면 `explore` 플래그를 쓸 수 없다.

**예시 A · Flutter**
```
- 기동: flutter build ios --debug --simulator → xcrun simctl install <UDID> build/ios/iphonesimulator/Runner.app → xcrun simctl launch <UDID> <bundle-id> (부팅 기기 없으면 xcrun simctl list devices available → boot <UDID>).
- 조작: 일회성 탐색 통합테스트(IntegrationTestWidgetsFlutterBinding)를 $ARTIFACTS_DIR/explore/ 에 작성해 `flutter test <파일> --timeout 120s -d <UDID>` 로 구동(동결 glob 밖 — 소모품). 스크린샷: xcrun simctl io <UDID> screenshot <path>.
- 초점: 주요 화면 진입/이탈 경로·빈 상태(데이터 0건)·뒤로가기 중첩·버튼 연타·백그라운드 전환 후 복귀.
```
**예시 B · Go**
```
- 기동: docker compose up -d && go run ./cmd/api (또는 어댑터 TEST_CMD 전제조건과 동일 환경).
- 조작: curl 시퀀스를 $ARTIFACTS_DIR/explore/ 스크립트로 작성·실행 — 명세 밖 순서(생성 전 수정·삭제 후 조회)·중복 요청·경계 밖 payload(빈 body·초과 길이·잘못된 타입/Content-Type)·인증 헤더 변조.
- 초점: 상태 전이가 있는 리소스(주문/예약 류)·페이지네이션 경계·동시 요청 경합.
```
**예시 C · Web**
```
- 기동: npm run dev (백그라운드) — 포트는 vite 기본 5173.
- 조작: 일회성 playwright 스크립트를 $ARTIFACTS_DIR/explore/ 에 작성해 `npx playwright test <파일> --reporter=line` 실행(라우트 순회·뒤로가기/새로고침·더블클릭·폼 이상 입력·빈/대량 데이터 상태). 스크린샷 증거를 같은 디렉터리에 저장.
- 초점: 멀티스텝 폼·라우트 간 상태 유지·로딩/에러 상태에서의 조작.
```
...
프로젝트의 성격·목표에 맞춰 **각 역할 에이전트의 초점을 조정**하는 섹션이다. `## ROLE_ARCHITECT`·`## ROLE_DESIGNER`·`## ROLE_TEST_AUTHOR`·`## ROLE_TEST_AUDITOR`·`## ROLE_IMPLEMENTER`·`## ROLE_REVIEWER`·`## ROLE_EXPLORER` 헤딩을 두면, 해당 에이전트가 시작 시 자기 섹션을 직접 Read 해 **프롬프트에 추가된 지침**으로 따른다(드라이버는 관여하지 않는다).
