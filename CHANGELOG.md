...
호출당 비용 최적화 릴리스 — v0.8(lean)이 **호출 횟수**를 줄였다면, v0.9 는 **호출 1회의 비용**을 줄인다. 근거는 실사용 프로젝트 1건의 전량 실측: 서브에이전트 733회 호출 = 73.7M 토큰, 그중 test-author(22.5M)+test-auditor(20.8M) = **59%**(카드당 평균 ~6회의 반려·환원 핑퐁), 재심사도 Read 무제한이면 최초 심사와 같은 비용(재심사 p10 6.7만 토큰), 호출당 SSOT+어댑터 재열람 고정비 누계 ~10M 토큰. 게이트 구조(동결·G2·mutation·GUARDS)는 그대로다 — 줄이는 것은 열람 범위와 산출물 부피뿐이다.

### Added
- **`skills/iterate-protocol/CORE.md`(에이전트 공통 커널)**: SKILL.md §불변식·§에이전트 독립성의 배포용 발췌 + 증분 회차 Read 스코프 규율 + 산출물 간결 규율. **역할 에이전트 7종은 이제 SSOT 통독 대신 이 커널만 읽는다**(SSOT 전문은 드라이버 몫). 동기화 규칙: SKILL.md 해당 절 수정 시 CORE.md 필수 동기화(정본은 SKILL.md).
- **test-author 증분 Read 스코프**: 환원·반려·보강 재호출에서 드라이버가 대상 파일·대상 AC 를 명시하고, test-author 는 대상 밖 동결 테스트 전문 열람 금지(확인은 Glob·Grep) — 증분 회차 중앙값 8.3만 토큰이 최초 작성과 같던 문제의 직접 대응.
- **test-auditor 델타 심사 모드 명문화**: 체크섬 불변 파일 **Read 금지**(직전 escapes-0 판정 유효 — 상호작용 확인은 목록·Grep), Mutant checklist 는 델타 AC 에서만, 산출물은 델타 스코프 한정·항목당 1~2줄.
- **결함 라운드 배치(SSOT §게이트 실패 0)**: 같은 게이트 라운드의 결함(테스트 fail·mutation 생존·reviewer 갭·explorer 결함)은 하나의 failures.md → 한 번의 재실행으로 — 결함 1건당 별도 회차 금지(재실행 1회 = test-author+G2 ≈ 20만 토큰).
- **test-author 선제 분할·명명 규칙**: 파일이 `FILE_LINE_LIMIT` 의 90% 를 넘보면 그 자리에서 분할(게이트 후발견 = 분할+G2 재심사 한 라운드 낭비, 실측 1회) + 신규 테스트 파일 basename 테스트 트리 전역 유니크(러너 수집 충돌 — 개명 재작업 실측 3회).
- **mutation 주입 패턴 규율**: 줄번호 지정 sed 금지 — `grep -n` 실측 후 패턴 매칭 Edit + 주입 직후 diff 확인(줄번호 어긋난 빈 변형 → 가짜 생존/무효 판정 실측 2회).
- **Explore(빌트인) 모델 지정**: 드라이버가 `model: "sonnet"` 으로 호출(코드 맵핑은 게이트 심사가 아니다 — 세션 opus 상속 불요).

### Changed
- 역할 에이전트 7종의 첫 Read 지시: `SKILL.md` → `CORE.md`(전체 SSOT 통독 금지 문구 포함).
- SKILL.md §GATE2 델타 심사: "기존 파일을 다시 읽는 것은 허용" → **불변 파일 Read 금지**(목록·Grep 한정)로 강화.
- README §1 구조·§6c 비용: CORE.md 항목 추가 + v0.9 실측 근거 문단 추가.

### 어댑터 영향
- **없음(필드 추가 불요)** — 전부 코어(플러그인) 내부 변경이다. 기존 어댑터·카드 원장 그대로 동작한다.
